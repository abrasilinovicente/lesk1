...
apt-get update -qq
apt-get install -y curl wget apt-transport-https gnupg lsb-release ca-certificates || true

echo "📦 Instalando dependências principais..."
apt-get install -y postfix dovecot-core dovecot-imapd opendkim opendkim-tools \
certbot python3-certbot-nginx nginx ufw jq unzip toilet || true

echo "🌐 Configurando hostname e firewall..."
hostnamectl set-hostname "$HOSTNAME"
ufw allow OpenSSH
ufw allow 25,80,143,443,465,587,993/tcp
ufw --force enable
...
# Configuração Cloudflare, se fornecido
